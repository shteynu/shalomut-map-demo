...
- Сначала запускай релевантные targeted tests.
- По необходимости запускай `npm test`, `npm run lint`, `npm run build`,
  Python tests и Prisma validation.
- Не считай `npm run dev` самостоятельным доказательством корректности.
...
1. Проверь `git status`, diff, commits и результаты выполненных проверок.
