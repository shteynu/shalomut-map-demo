...
| 5. Presentation и hardening | Частично: представление и OpenAPI закрыты, identity нет |
...
Нет: enum/CHECK-констрейнтов на остальных таблицах и перевода identity на
Argon2 или managed IdP — пароль всё ещё хешируется SHA-256 со статическим
перцем.
...
закрыты; из списков «Нет» выше открытым остался только этап 5: identity на
Argon2 или managed IdP и enum/CHECK-констрейнты на остальных таблицах.

Крупнейший независимый слайс из оставшегося — identity: пароль всё ещё
хешируется SHA-256 со статическим перцем, и этот же пункт поглощает переход от
organization-scoped shared gate к настоящей tenant authorization (см. C5).
