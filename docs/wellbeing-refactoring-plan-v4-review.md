...
| 5. Presentation и hardening | Частично: представление закрыто, identity и OpenAPI нет |
...
Нет: генерации OpenAPI из одного источника (по-прежнему два редактируемых
артефакта с integrity-тестом), enum/CHECK-констрейнтов на остальных таблицах и
перевода identity на Argon2 или managed IdP — пароль всё ещё хешируется SHA-256
со статическим перцем.
...
Все строки таблицы влиты в `main` 2026-08-03. Этапы 0–4 закрыты; из списков
«Нет» выше открытым остался только этап 5: identity на Argon2 или managed IdP,
генерация OpenAPI из одного источника и enum/CHECK-констрейнты на остальных
таблицах.
