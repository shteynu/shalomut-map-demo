...
Ближайший неоткрытый вопрос из этой работы: `stone_map_refusal` написан, но в
pipeline не подключён. Подключение превращает `contract_validation_failed` —
тупик, в котором раунд вызовов модели уже оплачен, а safety-цикл давно
завершился, — в ещё один replay. Это смена поведения и отдельный слайс.
