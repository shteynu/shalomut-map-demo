...
| 5. Presentation и hardening | Частично: представление и OpenAPI закрыты, identity отложена решением владельца |
...
валидатор собственного исходящего payload'а (`stone_map_refusal`), и следующим
слайсом он подключён к графу — см. этап 4.
...
не понесёт, не должна за неё платить. Прежняя редакция этого абзаца обещала, что
порт `TextGenerator` из этапа 4 закроет и это. Порт сделан (`612b4fb`), и не
закрыл: он называет, кто пишет копию, но не решает, какую копию просить. Оба
вопроса остались в нодах, и это правильное для них место.
...
Закрыто после аудита: `TextGenerator` назван портом, и обе async-ноды берут
генератор параметром, а не именем модуля. Сервис не поехал ни в state-контракт,
ни в методы класса — граф держит одного коллаборатора, которого нода не может
достать из состояния, и передаёт его вызовом. Дефолт остался прежним провайдером,
поэтому ни один существующий вызов не изменился. Проверяется раундом, который
проходит целиком на подставном генераторе, пока все пять генерирующих методов
синглтона подменены на исключение.

Что этот слайс не сделал, вопреки предыдущей редакции раздела: последний вопрос
к capabilities внутри нод остался на месте. Он не про провайдера — ноды
спрашивают контракт, чтобы решить, какую копию писать, а порт отвечает только на
вопрос, кто её пишет.
...
результат по-прежнему лежит в колонках `survey_rounds`, миграции нет.
Отказ записи для несуществующего раунда остался частью контракта: Postgres даёт
его провалившимся update, in-memory — вопросом к round store, и именно он держит
404 на callback для неизвестного раунда.
...
Отдельно всплыло, что safety-валидатор судит только состояние — копию, которую
написали ноды. Сам payload собирался после цикла ретраев, и его не проверял
никто, поэтому раунд мог пройти здесь все проверки и быть отвергнут Core на
callback, где вызовы модели уже оплачены, а ретраев не осталось. Конкретный
пример: у валидатора нет мнения о числе предложений в сводке раунда, а Core с
5.0 требует 2–4. Теперь собранный payload проверяется до отправки, причём отказ
сначала классифицируется: шесть правил уровня копии становятся обычным replay с
critique, остальные валят раунд сразу с именем правила — переигрывать
`surveyDefinitionHash`, которого не было во входных данных, значит купить
heavy-запрос ради того же самого отказа.

### Этап 5

Есть: observability джобов почти по всему списку §12.6 и CHECK-констрейнты с
индексами для `ai_analysis_runs`.

Закрыто после аудита: `DashboardInsightsDto` как явный контракт представления.
Прежняя формулировка «UI фактически не знает версий контракта» была слишком
доброй: компоненты держали в состоянии сам `StoneMapResult`, а
`dashboard-map-page.tsx` читал `overallPsychologicalSummary` — имя поля с
провода — прямо на экране. Теперь `toDashboardInsights` в
`ai-insights-view-model.ts` — единственный перевод с провода, ниже него
экраны видят только DTO, и ни один компонент не импортирует `ai-contract`.

Тем же движением закрыт вынос production-типов из `demo-data.ts`: файла больше
нет. `WellbeingDimension` смешивал две разные вещи — постоянную презентацию
измерения (подписи, геометрия карты, цвет) и результат раунда (score, status,
summary, метрики, рекомендации). Из-за второй половины в production-модуле жило
около 500 строк захардкоженной ивритской demo-аналитики, которую не рендерил ни
один экран: при отсутствии stone страницы показывают AI-состояние. Презентация
уехала в `src/lib/dashboard/dimension-presentation.ts`, остальное стало
`DashboardStone`. Легаси-компонент `stone-map.tsx` удалён — он экспортировался
из barrel, не рендерился нигде и был единственным потребителем demo-очков.

Подписи оставлены как были, включая расхождение с `surveyInstrument`
(`self-expression` — «קול אישי» в презентации и «ביטוי עצמי» в каноническом
источнике). Свести их — продуктовое решение, а не рефакторинг.

Нет: enum/CHECK-констрейнтов на остальных таблицах.

Identity закрыта не кодом, а решением владельца 2026-08-03: менеджер на
деплой один, второго требования нет, поэтому долгосрочная модель identity
становится requirement-gated будущей фичей, а не открытой архитектурной
задачей. Пункт «перевести на Argon2» отдельно снят как ложная цель: хеш
выводится из `MANAGER_ADMIN_PASSWORD` на каждый логин и нигде не хранится, так
что memory-hard KDF защищал бы несуществующую базу хешей. Подробности —
`PROJECT_CONTEXT.md` ADR-013 и `docs/product-behaviour-backlog.md` §8.

OpenAPI закрыт отдельным слайсом: `docs/openapi.yaml` — единственный
редактируемый артефакт, `public/openapi.json` генерируется из него. Прежний
integrity-тест сравнивал список из 32 имён AI-схем, поэтому всё остальное могло
разойтись незамеченным — и разошлось: описание `/api/rounds/{roundId}/reset`
отличалось между двумя файлами. Сравнивается теперь весь документ целиком.
...
Единственное место в разделе, где говорится о состоянии веток. Открытых веток
из этого списка нет — всё ниже влито в `main`:
...
| `0b57e6b` | направление callback в общем корпусе плюс `stone_map_refusal` на стороне сервиса |
...
| `4802b22` | composition root в Core: `getRepositories()` удалён, проводка живёт в одном модуле, границу держит fitness-функция |
| `906f2ba` | `DashboardInsightsDto`: экраны держат DTO вместо `StoneMapResult`, `demo-data.ts` удалён вместе с demo-аналитикой |
| `7d60b59` | OpenAPI из одного источника: `public/openapi.json` генерируется из YAML, список из 32 схем заменён сравнением всего документа. Ветка `refactor/openapi-single-source`, ещё не в `main` |

Все строки таблицы влиты в `main` 2026-08-03, кроме последней. Этапы 0–4
закрыты; из списков «Нет» выше открытыми остались только enum/CHECK-констрейнты
на остальных таблицах.

Identity из этого списка вышла решением владельца, а не реализацией, и вместе с
ней — поглощённый ею переход к настоящей tenant authorization (см. C5). Оба
пункта возвращаются в работу, когда появится второе требование: второй менеджер
на школу, мультиарендность или настоящие респонденты.
