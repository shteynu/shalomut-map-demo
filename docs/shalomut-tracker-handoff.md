...
1. **Ротация четырёх секретов (пункт 12)** — приоритет, руки владельца. Порядок и почему он важен — в снапшоте
   ниже, в пункте про засвеченные секреты.
2. **Задеплоить Python раньше Core (пункт 8)**. У AI-сервиса появился новый эндпоинт
   ...
   `AI_WEBHOOK_SECRET`.
3. **Живой раунд** как только он естественно случится: он проверит частичную карту (пункт 15) сразу, а темп
...
4. **Первый раунд после деплоя стоит прочитать `inspect-ai-provenance`** (пункт 10). Модель теперь пишет и
   зелёные измерения, так что 8/8 `llm` перестало быть признаком раунда без зелёных. Скрипт различает две дороги
   к детерминированной фразе по числу попыток и пишет это в строке.
5. Тесты Python запускать интерпретатором из venv: `.venv/bin/python -m pytest` из `ai-analytics-service`.
