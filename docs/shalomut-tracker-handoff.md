...
Обновлено: 2026-07-28 (найдена причина deterministic_fallback; порог приватности 10, теперь и в базе; код всё ещё в неслитой ветке)
...
- **Ветка `feature/ai-insights-depth-v5` не запушена и не задеплоена (2026-07-28)**: 36 коммитов поверх
  `origin/main` (`0a816e3`). Локальный `main` стоит ровно на `origin/main`. Push пробовали дважды 2026-07-28,
  второй раз — по прямой просьбе владельца; оба раза его отклонил слой разрешений сессии. Остаётся за владельцем. Всё описанное ниже в этом пункте живёт только в ветке — deployed
  runtime по-прежнему ведёт себя как `main`. **Исключение — миграция порога: она применена к базе и действует
  уже сейчас**, потому что база общая, а `main` читает колонку напрямую.
...
- **Заперт единственный раунд.** У `SHALOM-F125` три ответа при пороге 10, поэтому любой локальный прогон по нему
  останавливается на privacy lock и до провайдера не доходит: MCP отдаёт `isLocked: true` с пустыми
  `dimensionScores`/`questionAggregates`, Python возвращает `locked_error`. Чтобы прогнать разблокированный
  путь, есть [`scripts/local-unlocked-pipeline.ts`](../scripts/local-unlocked-pipeline.ts): он собирает раунд с
