...
- **Первый живой раунд на задеплоенном стеке упал по провайдеру (2026-07-29)**. Раунд `f9c18f1c` с десятью
  ...
  - **Причина не установлена, и `provider_unavailable` её не называет.** Под этот `failureReason` попадают три
    разных случая: квота (`429`), таймаут и вывод, отвергнутый валидаторами на всех попытках. Последний — это
    ровно то, как выглядит `MAX_TOKENS_PER_DIMENSION=420`, если он всё ещё задан явно в дашборде Render:
    обрезанный ответ, `finish_reason: "length"`, честная отбраковка. Различить можно только по логу сервиса на
    Render, где пишется `outcome=provider_unavailable reason=…`. Это следующий шаг, и он требует доступа
    владельца к Render.
