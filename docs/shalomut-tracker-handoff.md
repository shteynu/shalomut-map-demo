...
Обновлено: 2026-08-02. В `main` объединены основной архитектурный рефакторинг,
lifecycle-aware UI после privacy threshold и follow-up по Python pipeline и
контрактам. GitHub Build & Validate
...
1. **Задеплоить Python раньше Core (пункт 8)**. У AI-сервиса появился новый эндпоинт
   ...
   ответит `503` с ивритской пояснительной строкой и предложит формулировку из шаблона — не поломка, но и не
   то, что стоит показывать. Новых переменных окружения нет: адрес выводится из `AI_SERVICE_URL`, секрет — тот же
   `AI_WEBHOOK_SECRET`.
2. **Один живой раунд с десятью ответами после обоих деплоев** закрывает deployment evidence для пунктов 8,
   10 и 15: использовать AI-подсказку в опроснике, пройти privacy-шлюз и прочитать результат через
   `inspect-ai-provenance`. Модель теперь пишет и зелёные измерения, так что 8/8 `llm` перестало быть признаком
   раунда без зелёных; скрипт различает две дороги к детерминированной фразе по числу попыток. Темп heavy
   (пункт 20) и узкий повтор (пункт 21) подтвердятся только если safety-валидатор естественно что-то отклонит.
3. **До первых настоящих респондентов ротировать четыре credentials (закрытое решение пункта 12).** Сейчас это
   не открытый риск: утечка была в переписку, реальных ответов нет, база расходная. С появлением настоящих
   ответов три shared secrets начинают охранять чужие данные, и условие перестаёт быть отсрочкой.
...
- **Модель пишет все восемь измерений, а зелёное перестали отклонять за обычный иврит (2026-07-30, пункт 10)**.
  ...
  которому провайдер отказал, сохраняет детерминированную фразу, а не превращается в пробел.
  - **Возражение про квоту оказалось пустым.** Живой раунд 2026-07-29 уже был раундом «все восемь пишет модель»:
    `8/8 outcome: llm` означает, что зелёных измерений в нём не было. 22 запроса, 97 секунд, ни одного `429` —
    измерение для той конфигурации, о которой спорил пункт, существовало ещё до его решения. Флаг стоит до пяти
    дополнительных запросов в раунде, где зелёные измерения есть, против 1000 в сутки.
  - **Чёрный список отклонял `שיפור`, `לשפר` и `שחיקה`, и первые два — это то, что школа делает с сильной
    стороной.** Они убраны из правила. `שחיקה` осталась, но только как утверждение: отрицание, стоящее перед ней
    в том же предложении, снимает запрет, поэтому `אין סימני שחיקה` — высказывание в пользу зелёного — больше не
    отклоняется за то, что называет то, что отрицает. Это то же различение, которое `5.0` уже делает для чужого
    цвета: разрешён как число, запрещён как вердикт. `מצוקה מבנית` и `טיפול מיידי` запрещены при любой
    формулировке — они противоречат счёту, которым владеет Core.
  - **Цена была не гипотетической.** У узла адаптации нет фильтра по цвету, поэтому зелёные рекомендации ходили
    к модели всё это время, и safety-валидатор применяет к переписанной записи тот же `is_status_consistent`.
    Зелёная переписка, потянувшаяся к слову `שיפור`, молча теряла адаптацию и оставалась текстом каталога — в
    каждом раунде, при включённом флаге. Сам каталог этих слов не содержит: проверены все 40 записей с
    `target_status: green`, ни одной. То есть наказывали только модель, и за обычное слово.
  - **Отказ на зелёном сохраняет фразу, а не оставляет пробел.** Зелёное — единственный статус, чья
    детерминированная фраза не догадка о проблеме: она выведена из агрегатов и это та же копия, которую зелёное
    получало, пока его вообще не спрашивали. `attempts` различает дороги — `0` не спрашивали, больше нуля
    спрашивали и получили отказ, — `outcome` остаётся `deterministic_fallback` и никогда не `llm`, а жёлтое с
    красным по-прежнему падают: там формула вместо анализа — ровно то, против чего существует fail-closed.
  - **`ONLY_LLM_FOR_PROBLEMATIC` наконец объявлен в `render.yaml`** — часть пункта, к решению не относившаяся:
    переменной не было ни в blueprint, ни в дашборде, и она по умолчанию была `true` в `config.py`, то есть
    решение не лежало нигде, где его можно прочитать. Дефолт в коде переехал в `false` вместе с ней, а blueprint
    перебивает дашборд (проверено на `LLM_MODEL_FAST`), так что одного деплоя достаточно в любом случае.
  - Проверка: `pytest` 269/269, 19 новых в `tests/test_green_dimensions.py`; fail-first подтверждён — против
    прежнего кода падают 8 из 19, остальные 11 обязаны проходить по обе стороны (вердикты по-прежнему
    отклоняются, отрицание внутри `לאורך` и `מלא` по-прежнему не отрицание, жёлтое с красным не тронуты, флаг
    по-прежнему умеет не звонить, а детерминированная фраза по-прежнему проходит валидаторы, которые её будут
    читать). Два старых теста фиксировали прежнее правило и переписаны, а не удалены. `npm test` 274/274,
    `eslint` чисто. **Живым раундом не проверено.**
  - **Остаточный риск, назван, а не устранён**: отказ safety-валидатора, переживший три повтора, по-прежнему
    убивает весь раунд на любой версии, включая `5.0` — частичная карта поглощает `provider_unavailable`, а не
    `validation_failed`. Зелёное добавляет до пяти измерений, которые могут туда попасть. Риск не новый и не
    специфичный для зелёного, а починка чёрного списка убирает триггер, делавший зелёное вероятнее прочих.
  - Порядок деплоя: только Python плюс значение в blueprint, контракт не менялся — порядок не важен.
- **Подсказка вопроса от модели, и её нельзя выдать за свою (2026-07-30, пункт 8 в «Next Up»)**. Библиотека
  вопросов в конструкторе была тремя захардкоженными формулировками на `bankCursor % questionBank.length` и
  покрывала три измерения из восьми — менеджер, собиравший раунд про остальные пять, начинал с пустого места.
  Теперь источников два: канонический инструмент (все восемь измерений) и запрос к AI-сервису. Три правила
  slice 3.1 выполнены, и каждое из них — тест.
  - **Запрос называет измерение.** `POST /api/manager/question-suggestion` (сессия менеджера, тот же
    middleware, что у остального manager API) просит у Python один היגד про одно из восьми измерений.
    `POST /api/v1/questions/suggest` сделан **вне версионированных контрактов аналитики** намеренно: в нём нет
    раунда, нет счётов и нет контекста школы, так что привязывать его форму к цепочке `5.0` не за что. Четвёртый
    секрет не заводился — используется `AI_WEBHOOK_SECRET`; адрес выводится из `AI_SERVICE_URL` через
    `new URL(path, base)`, поэтому новой переменной окружения, которую можно забыть, не существует.
  - **Источник помечен, и пометка правдива на всём пути.** Python отвечает `source: "llm"` только про то, что
    написала модель; Core ставит `source: "ai"` только на такое, а `200` с любым другим источником отклоняет как
    `invalid_response` — детерминированная строка не носит ярлык модели здесь по той же причине, по которой не
    носит его в камне. Вторая кнопка подписана тем, чем она является: формулировка из исходного шаблона
    опросника. Тест OpenAPI фиксирует `QuestionSuggestion.source.enum` как `['ai']`, чтобы потребителю нельзя
    было пообещать `template` с этого маршрута.
  - **Ручная правка обязательна.** Диалог открывается с предложенной формулировкой и не даёт добавить её, пока
    текст равен предложенному; причина — ивритским текстом и через `aria-describedby`, а не только disabled и
    цветом. Правило распространяется и на шаблонную формулировку: школа читает опросник как слова менеджера.
  - Мелочи, которые пришли вместе: формулировка, уже лежащая в черновике, второй раз не предлагается (сравнение
    нормализованное — пробелы и пропавшая точка это тот же היגד), а отказ AI не тупик — предлагается шаблон с
    пояснением в той же панели, где нажали кнопку.
  - Проверка: `pytest` 250/250 (16 новых), `npm test` 274/274 (23 новых), `eslint` чисто. Изменённые
    поверхности не только проверены ассертами, но и просмотрены: конструктор за авторизацией менеджера, поэтому
    панель и диалог отрендерены в статическую страницу и шесть состояний осмотрены в RTL — покой, запрос в
    полёте, AI недоступен с предложенным шаблоном, неотредактированная подсказка, отредактированная и обычная
    правка без ярлыка. Именно этот осмотр нашёл то, чего тесты найти не могли: кнопка AI была третьим тёмным
    primary рядом с настоящим primary страницы («сохранить опросник») и стала тонированным secondary.
    **Против деплоя не проверено** — деплой ещё не просил подсказку.
  - **Порядок деплоя: Python первым.** Новый эндпоинт у Python, Core — единственный вызывающий.
...
- **Решение по четырём credentials, засвеченным в переписке (2026-07-30), принято и закрыто**. Ключ Gemini и
  три общих секрета между Core и AI-сервисом — `MCP_SHARED_SECRET`, `AI_WEBHOOK_SECRET`,
  `AI_CALLBACK_SECRET` — сейчас не ротируются. Экспозиция ограничена перепиской, публичного repository leak не
  было, реальных респондентов и ценных данных нет. Принятый практический радиус: владелец transcript может
  запустить анализ, прочитать агрегаты раунда или подделать callback. Жёсткое условие решения: ротировать все
  четыре credentials **до первых настоящих респондентов**. Тогда три shared secrets менять по одному, Render и
  Vercel подряд; между парными записями callback и MCP-чтение упадут fail-closed, и идущий раунд умрёт. Ключ
  Gemini независим и меняется в Google AI Studio. Ни одно значение в репозиторий не попало. Пункт 12 закрыт.
...
- **Открытое решение владельца** (**снято 2026-07-30, первым пунктом снапшота выше**): пускать ли модель и на
  зелёные измерения (`ONLY_LLM_FOR_PROBLEMATIC=false`). Переключатель есть, кода не требует, но сначала нужно
  снять три вещи — чёрный список слов для `green` (`שיפור`/`לשפר`/`שחיקה` — обычная речь о сильной стороне),
  цену отказа (сейчас несостоявшийся вызов роняет весь раунд), и квоту. Подробности — пункт 10 в «Next Up»
