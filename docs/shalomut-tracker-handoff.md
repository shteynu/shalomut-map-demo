...
- **The four audit entries that were stranded on a branch are on `main` and
  ...
  a breakdown cell that says how many people are behind it (`91f4c8b`,
  `b78a9fb`), a login redirect checked after the browser would parse it
  (`92d8cc2`, `f906406`), an anonymous submission that carries a session and
  meets a ceiling (`bfbfdf9`, `25858a9`), and a verified database certificate
  (`5309de9`, `3fed143`). One thing was checked on the deployment itself, and it
  is the one that could have taken the site down: the deployed runtime still
  reaches its database now that the pool verifies the certificate.
  `GET /api/survey/ZZZZZZZZ/` answers `404` with `Survey round with code
  'ZZZZZZZZ' not found` — that is a query result, where a refused TLS handshake
  would have been the route's `500`. **The other three are present in the
  deployed tree and were proved locally; none of them has been walked on the
  deployment.** Branches land here with `git push origin <branch>:main`; a plain
  `git push origin <branch>` publishes the branch and lands nothing.
