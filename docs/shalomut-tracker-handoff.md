...
- **Частичная карта: одно мёртвое измерение больше не выбрасывает семь живых (2026-07-30, коммит `fe2a6fb`,
...
- **Быстрый путь переехал на `gemini-3.5-flash-lite` (2026-07-29; задеплоено и проверено живым раундом)**. Лимиты считаются
  ...
  - **Известный край, не закрыт**: темп — одно число на процесс, а лимиты на модель, поэтому повтор узла
    (единственный путь к `LLM_MODEL_HEAVY`) пойдёт на 14 в минуту против пяти у flash. Повторы редки и падают
    fail-closed; настоящее решение — очередь на модель.
