...
Обновлено: 2026-07-30 (**первый живой раунд завершился `success`, все восемь камней написаны моделью**;
квота больше не блокер; отказы адаптации разобраны и исправлены; частичная карта и опубликованные контракты
`4.0`/`5.0` доделаны локально и живым раундом не проверены; открыто и требует решения владельца:
ротация четырёх секретов — они засвечены дважды, включая переписку вечера 2026-07-29)
