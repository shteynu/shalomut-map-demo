...
Обновлено: 2026-07-28 (найдена причина deterministic_fallback; порог приватности 10; всё это в неслитой ветке)
...
- **Ветка `feature/ai-insights-depth-v5` не запушена и не задеплоена (2026-07-28)**: 31 коммит поверх
  `origin/main` (`0a816e3`). Локальный `main` стоит ровно на `origin/main`. Всё описанное ниже в этом пункте
  живёт только в ветке — deployed runtime по-прежнему ведёт себя как `main`.
  ...
  - **Порог приватности стал 10 везде** — минимум и дефолт в Core, fallback и clamp в Python-сервисе,
    ...
    `privacy_threshold` читается только через `effectivePrivacyThreshold`. **Следствие, которое владелец ещё не
    решил**: `SHALOM-F125` создан с порогом `1` и имеет 3 ответа, значит после деплоя его дашборд и инсайты
    станут читаться как locked. Либо это и есть намерение, либо таким раундам нужна миграция.
