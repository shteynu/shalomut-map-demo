...
- **Core переключён на контракт `5.0` (2026-07-29; выполнено владельцем)**. `AI_ANALYTICS_CONTRACT_VERSION`
  ...
  для locked-раунда, — но в базе нет ни одного раунда с 2026-07-28, поэтому сначала нужен пункт 4 «Next Up».
...
- **Фолбэк больше не выдаёт себя за анализ (2026-07-28, решение владельца; локально, не задеплоено)**.
  ...
  - Осталось владельцу: выкатить обе половины (Python и Core можно в любом порядке — см. выше про контракт) и,
    если нужен зелёный сквозной прогон, снять ограничение квоты Gemini: free tier отдаёт `429` уже на первых
    вызовах, а один раунд — примерно 33 вызова.
