...
Обновлено: 2026-07-28 (сбой провайдера больше не подменяется текстом сервиса: раунд честно падает и UI говорит
на иврите, что сервис анализа недоступен; сделано локально, не задеплоено)
...
- **Нечем прогнать конвейер по-настоящему.** База пуста, а до очистки единственный раунд `SHALOM-F125` имел три
  ...
  потому, что ключа провайдера в окружении не было (`llm_provider.py:248` → `missing_api_key`). Для живой модели
