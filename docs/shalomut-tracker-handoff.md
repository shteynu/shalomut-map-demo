...
Обновлено: 2026-07-29 (вызовы провайдера в раунде ограничены двумя одновременными, `llm_provider.py` разделён
на транспорт / промпты / валидацию; смержено в `main` как PR #13 и живёт на Render — `/health` отдаёт
`commit: 10c94ff`)
...
- **Ветка `feature/ai-insights-depth-v5` слита и задеплоена (2026-07-28)**: PR
  ...
  когда так говорит `AI_ANALYTICS_CONTRACT_VERSION`. Переменная заведена и в Production, и в Preview; значение
  зашифровано и в этой сессии не читалось, по записи от 2026-07-27 там `4.0`.
