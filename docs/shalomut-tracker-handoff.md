...
Обновлено: 2026-07-30, сессия закрыта (**первый живой раунд завершился `success`, все восемь камней написаны
моделью**; квота больше не блокер; закрыты пункты 15–20; всё запушено, `origin/main` на `01fd852`, дерево
чистое. Пункты 8 и 10 оставлены открытыми решением владельца. Единственный открытый риск — ротация четырёх
...
2. **Живой раунд** как только он естественно случится: он проверит частичную карту (пункт 15) сразу и подтвердит
   темп heavy (пункт 20) только если safety-валидатор отклонит узел целиком, чего по заказу не устроить.
   `LLM_MAX_REQUESTS_PER_MINUTE_HEAVY=4` уедет на Render следующим деплоем через `render.yaml`.
...
- **Очередь к провайдеру стала по модели, а не одна на процесс (2026-07-30, пункт 20 в «Next Up»)**. Темп был
  ...
  - **Остаточный риск, который темпом не лечится**: у heavy 20 запросов в сутки, то есть четыре повтора
    ...
    повтор перестал отправлять на heavy весь узел разом. Живым раундом не проверено и по заказу не проверяется:
    heavy достигается только когда safety-валидатор отклоняет узел целиком.
