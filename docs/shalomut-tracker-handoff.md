...
- **Первый живой раунд на задеплоенном стеке упал по провайдеру (2026-07-29)**. Раунд `f9c18f1c` с десятью
  ...
  - **Обрезание ответов — вторая, отдельная.** До первого `429` были две отбраковки
    ...
    `finish_reason != "stop"`, то есть на оборванном ответе — сигнатура `MAX_TOKENS_PER_DIMENSION=420`. В
    [`config.py:151`](../ai-analytics-service/src/config.py) дефолт `2048`, но он не применится, если
    переменная задана в дашборде Render явно. **Эта проверка всё ещё не сделана и остаётся ближайшим шагом.**
...
- **Ветка `feature/ai-insights-depth-v5` слита и задеплоена (2026-07-28)**: PR
  ...
  - **Перед деплоем проверить в дашборде Render**, не задан ли `MAX_TOKENS_PER_DIMENSION` явно: ни
    `render.yaml`, ни `.env.render.local` его не задают, поэтому новый дефолт применится только если переменной
    там нет. Второе — квота Gemini: на free tier `429` приходит уже через несколько вызовов, а один живой раунд
    это примерно 33 вызова.
