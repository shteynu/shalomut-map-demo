...
- **Первый живой раунд на задеплоенном стеке упал по провайдеру (2026-07-29)**. Раунд `f9c18f1c` с десятью
  ...
  - Не объяснено и оставлено: в логе сервиса разрыв `10:44:36`–`11:09:37` (`Shutting down` → `Deploying…`),
    внутри которого прошёл первый раунд. `AI_SERVICE_URL` проверен — сервис тот самый,
    `shalomut-ai-analytics.onrender.com`. Трасса `11:17` перекрывает вопрос полностью, дальше не копали.
