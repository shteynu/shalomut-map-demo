...
- **Границы**: остаётся детерминированная копия там, где вызова провайдера нет
  вовсе (зелёное измерение при `ONLY_LLM_FOR_PROBLEMATIC`, провенанс
  `deterministic_fallback`/`attempts=0`) и текст каталога, написанный человеком,
  при неудачной адаптации рекомендации на `5.0`. Ни то ни другое не прикрывает
  упавший вызов.
