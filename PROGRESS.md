...
Dashboard DTO under `AI_ANALYTICS_CONTRACT_VERSION=7.0`. **Both services are
deployed with it since 2026-09-13** — the service serves `effd7b1`, Core
`a0e245d` with the variable switched to `7.0` — and the unset producer default
stays `5.0`. **The swap
...
snapshot. While the deployment produced `6.0`, `encodeAnalyticsInput` refused
to analyse a round on the instrument — closed, at the boundary, naming the
variable to change; it produces `7.0` now, and the first analysed round on
the instrument waits on the provider credit. The eight-branch stack **landed on `main` on 2026-08-15** as one
