  ...
  implements ISurveyDefinitionVersionRepository
{
   ...
   * Measured locally over twenty versions, warm, averaged over forty runs, on
   * the 2026-08-23 shape of this query: 2.6 ms against 1.6 ms at 24 questions
   * per version, 7.0 ms against 3.4 ms at 126, with the result shrinking from
   * 132 KB and 640 KB to 2.4 KB. The database this talks to in the deployed
   * environment is not in the same continent as the process reading it, so
   * the local number is a floor.
   *
   * Tagged template, so `roundId` is a bound parameter and never text in a
   * statement.
   *
   * The counts are items, as `countQuestionnaireItems` counts them: a grid —
   * a background question in `allocation-100` mode with a group id — is one
   * item however many rows it has, so the rows are counted by distinct group
   * and everything else per question. The three predicates are wrapped in
   * `coalesce` because a missing key would otherwise make the whole `CASE`
   * condition `NULL`, and a `NULL` there must read as "not a grid row" —
   * every legacy question is missing `kind`. `count(*)` is cast to `int`
   * because a `bigint` arrives as a JavaScript `BigInt`, which
   * `JSON.stringify` refuses; a questionnaire will not overflow it.
   * `jsonb_array_elements` is strict, so a row whose definition somehow
   * carried no `questions` array expands to nothing and both counts read as
  ...
  public async findSummariesByRoundId(
    ...
    >`
      SELECT
        "version"."id",
        "version"."saved_at",
        "version"."definition" ->> 'title' AS "title",
        "items"."question_count",
        "items"."enabled_question_count"
      FROM "survey_definition_versions" AS "version"
      CROSS JOIN LATERAL (
        SELECT
          count(*) FILTER (WHERE "grid_id" IS NULL)::int
            + count(DISTINCT "grid_id")::int AS "question_count",
          count(*) FILTER (WHERE "enabled" AND "grid_id" IS NULL)::int
            + (count(DISTINCT "grid_id") FILTER (WHERE "enabled"))::int
            AS "enabled_question_count"
        FROM (
          SELECT
            ("question" ->> 'enabled')::boolean AS "enabled",
            CASE
              WHEN coalesce("question" ->> 'kind', '') = 'background'
                AND coalesce("question" ->> 'answerMode', '') = 'allocation-100'
                AND coalesce("question" ->> 'allocationGroupId', '') <> ''
              THEN "question" ->> 'allocationGroupId'
            END AS "grid_id"
          FROM jsonb_array_elements("version"."definition" -> 'questions')
            AS "question"
        ) AS "row"
      ) AS "items"
      WHERE "version"."round_id" = ${roundId}
      ORDER BY "version"."saved_at" DESC, "version"."id" DESC
    `;
